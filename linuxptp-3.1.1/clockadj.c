...
void clockadj_init(clockid_t clkid)
{
	...
	pr_info("Realtime Hz %ld", realtime_hz);
	pr_info("Realtime nominal tick %ld", realtime_nominal_tick);
	}
#endif
}
...
void clockadj_set_freq(clockid_t clkid, double freq)
{
	...
	pr_info("I Just set the clock frequency! clk_id = %d, fre = %f", (int)clkid, freq);
}
...
void clockadj_set_phase(clockid_t clkid, long offset)
{
	...
	pr_info("I Just adjusted the clock phase!");
}
...
void clockadj_step(clockid_t clkid, int64_t step)
{
	...
	pr_info("I Just stepped the clock! clk_id = %d, step = %lu", (int)clkid, step);
}
