...
struct servo *pi_servo_create(int fadj, int sw_ts)
{
	...
	// Config is read from config.h statically.
	// Modified by Tibor Tusori
	// 
	...
	s->configured_pi_kp 			= PI_PROPORTIONAL_CONST;
	s->configured_pi_ki 			= PI_INTEGRAL_CONST;
	s->configured_pi_kp_scale 		= PI_PROPORTIONAL_SCALE;
	s->configured_pi_kp_exponent  	= PI_PROPORTIONAL_EXPONENT;
	s->configured_pi_kp_norm_max 	= PI_PROPORTIONAL_NORM_MAX;
	s->configured_pi_ki_scale 		= PI_INTEGRAL_SCALE;
	s->configured_pi_ki_exponent 	= PI_INTEGRAL_EXPONENT;
	s->configured_pi_ki_norm_max 	= PI_INTEGRAL_NORM_MAX;
